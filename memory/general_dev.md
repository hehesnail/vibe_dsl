...
- 当 semantic recovery 需要区分 `index_state` / selection-like state 这类角色时，不要回退到 buffer 名字匹配。应把所需结构信号直接放进上游 typed analysis attrs，例如在 `blackhole.fragment_regions[*].fragment_buffers[*]` 上显式携带 `is_integer`
