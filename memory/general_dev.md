...
- 当某个阶段文档已经从“执行清单”降级成“边界参考”后，也要同步把里面的
  checklist、单次验证数字、Task/Step 计划体压掉；否则根目录 active docs
  很快又会退化成历史施工记录
...
- 阶段设计文档不要长期混入“本轮 hardening slice / 本轮验证 / 当日实现流水账”。
  这些内容短期有用，但会很快把长期边界文档污染成历史记录。
  更稳的做法是：
  - 阶段文档只保留稳定边界、未完成 contract、退出条件
  - 实现流水账和一次性验证结果收敛到 `progress.md` 或提交历史
