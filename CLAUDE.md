...
□ 2. 经验总结: 将可复用模式写入 memory/general_dev.md
   - 新发现的开发模式
   - 环境配置技巧
   - 代码组织方式
□ 3. 问题记录: 将遇到的问题写入 memory/bugs.md
   - 问题描述
   - 根本原因
   - 解决方案
□ 4. 更新设计: 确保 tasks/dev_design/ 与实际实现一致
   - 如有偏差，更新设计文档
   - 记录最终决策理由
□ 5. 提交推送: git commit & push 所有变更
   - 代码变更
   - 文档更新
   - 独立提交，不要与代码混在一起
```

**⚠️ 重要**: 这一步容易遗漏！请逐条检查，不要假设"应该已经完成了"。
...
| # | 检查项 | 目标文件 | 内容要求 |
|---|--------|----------|----------|
| 1 | 任务状态 | `tasks/progress.md` | 标记"已完成" |
| 2 | 经验总结 | `memory/general_dev.md` | 新增可复用模式 |
| 3 | 问题记录 | `memory/bugs.md` | 记录遇到的问题和解决方案 |
| 4 | 设计同步 | `tasks/dev_design/` | 与实际实现一致 |
| 5 | Git 提交 | 远程仓库 | 代码+文档分别提交 |

**常见遗漏**：第 2、3 项最容易忘记！请主动检查是否需要更新。

**自检方法**：
1. 查看本次任务修改了哪些文件
2. 思考：有哪些经验值得记录？
3. 思考：遇到了哪些问题？
4. 打开对应的 memory 文件，确认是否需要追加内容
