...
- **补充设计进展**: 已新增 `tasks/dev_design/stage4_flash_attention_forward_subset.md`，把 Blackhole 支持 `example_mha_fwd_bshd.py` / `example_gqa_fwd_bshd.py` 前向完整语义的方案，收敛为“IR 优先、spec 最小化”的通用 work decomposition / fragment compute region / pipelined staging 三类分析与 lowering 设计；Task 2 已完成首个 split-after analysis pass `AnalyzeBlackholeWorkDecomposition`，当前 `test_blackhole_flash_attention_analysis.py -k work_decomposition` 已通过，整份 analysis 文件剩余 `fragment_regions` / `pipeline_stages` 两项待实现
- **日期**: 2026-03-31
