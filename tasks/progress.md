...
- **补充设计进展**: 已新增 `tasks/dev_design/stage4_flash_attention_forward_subset.md`，把 Blackhole 支持 `example_mha_fwd_bshd.py` / `example_gqa_fwd_bshd.py` 前向完整语义的方案，收敛为“IR 优先、spec 最小化”的通用 work decomposition / fragment compute region / pipelined staging 三类分析与 lowering 设计；当前 split-after analysis 三件套已全部落地：`AnalyzeBlackholeWorkDecomposition`、`AnalyzeBlackholeFragmentRegions`、`AnalyzeBlackholePipelineStages` 已进入 `SplitBlackholeKernel` 之后的 Blackhole lowering 主链，`test_blackhole_flash_attention_analysis.py` 当前 `3 passed`。本轮已进一步收正 Blackhole `lower()` 主链边界：`is_device_call()` 现在会把 Blackhole entry `PrimFunc` 视为 device 输入，避免在 `SplitBlackholeKernel` 之前把 `main` 过滤掉；同时 flash-attention forward 的 target-level 红灯已从晚期 codegen `Find undefined Variable acc_o` 前移到 `LowerBlackholeOps` 的显式 fragment-subset fail-fast，`test_blackhole_flash_attention_pipeline.py` 当前 `1 passed`。现阶段新的主要 blocker 已转成 Task 5/6 的实做：不是再追 staged-copy 误判，而是把 fragment/pipeline analysis 结果真正消费进更宽的 lowering / legality
- **最新实现进展**: `AnalyzeBlackholeFragmentRegions` 已进一步收正到 split-after MHA/GQA 的真实 TIR 形态：row-wise reduction / scalar-to-vector row broadcast 不再只依赖 `CallNode` 和索引 rank 差，而是显式识别 `AddNode/MaxNode/MulNode/DivNode` 这类原生 TIR 表达式节点，以及 scalar fragment -> vector fragment 的广播关系。`test_blackhole_flash_attention_analysis.py` 现为 `4 passed`，其中 MHA / GQA 两条 fragment-region case 都能稳定暴露 `gemm + row_reduction + row_broadcast + pointwise_chain`；`test_blackhole_flash_attention_pipeline.py` 继续保持 `1 passed`。当前下一步不再是补 analysis，而是进入 Task 5/6，把 fragment / pipeline analysis 结果真正消费进 `LowerBlackholeOps` 与后续 legality
- **最新主链推进**: `LowerBlackholeOps` 现已开始正式消费 split-after analysis，新增通用 `blackhole.lowering_requirements` IR attrs，总结 work axes、fragment op kinds / reduction targets / broadcast sources、以及 pipeline stage counts / loop vars；没有新增 `flash_attention_plan` / `attention_work_contract` 一类 attention 专属 schema。与此同时，当前“不支持的 fragment compute subset” 显式 gate 已从 `LowerBlackholeOps` 迁移到 `rt_mod_blackhole` 的 build-time 检查：主链现在会先完成 analysis → lowering requirements 归一化，再在 runtime module 抽取前按 `row_reduction/row_broadcast/pointwise_chain` fail-fast。`test_blackhole_flash_attention_pipeline.py` 当前 `2 passed`，copy/gemm 既有 metadata 子集回归仍保持通过。下一步进入真正的 Task 6/后续 lowering：不是再补 summary attrs，而是把其中一部分 generic lowering requirements 继续变成可执行的 legality / lowering 结论
- **最新 legality 收口**: `LowerBlackholeOps` 现已新增第一条 generic fragment-pipeline legality：直接从 loop `num_stages` 注解做 build-time gate，当前 `stage_count > 2` 会在 `LowerBlackholeOps` 入口显式失败，错误口径为 `Blackhole fragment pipeline legality: unsupported stage count <N>`。对应回归已加入 `test_blackhole_flash_attention_pipeline.py`；当前这条回归先锁在 `LowerBlackholeOps` 直接入口而不是 full `lower()`，因为 GQA `num_stages=4` 形态在更深的 GEMM region canonicalization 上还会先触发独立的 `RegionOp` 内部错误。也就是说，generic legality 已建起来，但 full target path 对更宽 GQA region 形态的清理还没完成
- **最新 GQA 差异收正**: GQA 更宽 pipeline（`num_stages=4`）的 fragment analysis 之前还漏掉 `row_broadcast`，根因不是 attrs 丢失，而是广播索引采用 `T.shift_right(i, k)` 这类 coarsened row-index 形态；analysis 现在已把 `tir.shift_right` 视作与 `floor_div` 同类的 row-broadcast 信号。新增的 `test_gqa_forward_wider_pipeline_still_exposes_row_broadcast_roles` 已通过，说明 GQA 更宽形态的 analysis 结果已更接近 MHA；剩余 blocker 继续收敛到更深的 GEMM region canonicalization，而不是前面的 analysis 丢语义
- **日期**: 2026-03-31
