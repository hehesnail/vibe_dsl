...
| T7 Exact-CB / materialization primitives | Complete | Exact-CB materialization, publication, consumer binding, and GEMM post-merge materialization are typed.  Seq64 bf16 flash-attn exact-CB partial combine now source/spec-admits through typed records; full GEMM/online-softmax direct runtime is gated by the current TT-Sim `t_tile_mmio_wr32` leaf-chain boundary. |
...
| T9 Workload first paths | In progress | T9.1 pre-grouped MoE/routed GEMM and the T9.3 dual-score MLA GEMM slice have bf16 direct-runtime correctness.  T9.2 paged GQA and T9.3 full paged MLA online-softmax paths are source/spec admitted but direct runtime is gated by the typed TT-Sim `t_tile_mmio_wr32` leaf-chain boundary.  T9.4-T9.6 are queued. |
...
- T9.2 paged GQA decode:
  keep the current source/spec admission on generic page-table and ragged
  `value_expr` bindings; isolate the full-runtime typed TT-Sim
  `t_tile_mmio_wr32` leaf-chain boundary from backend schema cleanup.
- T9.3 paged MLA decode:
  keep the admitted page-table/ragged bindings, retained latent-KV lifetime,
  and dual-score GEMM correctness.  The full online-softmax decode remains
  source/spec admitted and shares the typed `t_tile_mmio_wr32` direct-runtime
  simulator gate with T9.2.
...
- T7/T9 current baseline: `cmake --build build -j32` passed; focused TT-Sim
  selector covering T7 seq64 exact-CB source/spec, extended-seq exact-CB
  metadata, T9.2/T9.3 source/spec admission, T9.3 dual-score GEMM direct
  runtime, small bf16 flash direct runtime, and T9.1 grouped GEMM direct
  runtime reported `7 passed, 4 skipped`.  The skipped full online-softmax
  paths carry the typed `GEMM/online-softmax ... t_tile_mmio_wr32` simulator
  reason; extended seq still carries the narrower loop-carried exact-CB PACR
  reason.
