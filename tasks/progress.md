...
- **最新实现进展**: flash-attn forward 的 `row_broadcast` 已开始进入真实 TIR lowering，而不再全部停在 build-time gate。`LowerBlackholeOps` 现在会把 optimized-path 上的 `acc_o[i] = acc_o[i] * scores_scale[0]` / `acc_o[i] = acc_o[i] / logsum[0]` 识别成最小 row-broadcast 子集，并 lower 成 `tl.blackhole.mul_row_bcast` / `tl.blackhole.div_row_bcast` builtin；对应 pipeline 回归已收进 `test_blackhole_flash_attention_pipeline.py`。当前 full `lower()` 仍会在 `rt_mod_blackhole` 因剩余未 lower 的 `row_broadcast` 形态显式 fail-fast，但 blocker 已从整类 row-broadcast 收窄到更复杂的融合广播/标量更新路径，而不是 `acc_o` 的简单标量广播更新。
