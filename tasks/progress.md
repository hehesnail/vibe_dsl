...
| T9 Workload first paths | In progress | T9.1 pre-grouped MoE/routed GEMM and T9.3 paged MLA have bf16 direct-runtime correctness.  T9.2 paged GQA is source/spec admitted but the full runtime path hits a typed PACR simulator boundary.  T9.4-T9.6 are queued. |
...
- T9.2 paged GQA decode:
  ...
  `value_expr` bindings; isolate the full-runtime
  `tensix_execute_pacr: intermediate_format=0 late_from_format=5` simulator
  boundary from backend schema cleanup.
