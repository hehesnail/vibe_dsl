...
buffer distribution remains the low-level address-ABI expansion point.
...
The table below names workload witnesses and repo evidence.  Those family
names and tensors are not semantic owners.  Irregular work must first be
analyzed from existing TIR loops, predicates, buffer loads/stores, and index
expressions; only durable evidence that cannot be recomputed downstream may be
projected into `SpatialPlan` or lowered into `TTProgram`.
...
| Grouped / ragged / sparse attention | `tilelang_repo/examples/blocksparse_attention/example_tilelang_sparse_gqa_decode_varlen_indice.py`; sparse GQA paged examples | Single-device sparse/ragged reads where index-list traversal and valid ranges are derived from TIR predicates and buffer access expressions; operands such as `block_indices` or `cache_seqlens` are not standalone metadata | Sharded sparse blocks, cross-core load balancing, distributed sparse scheduling |
...
| Multi-block flash-attn / flash decode | Current flash-attn Blackhole tests; `tt_metal_repo/tech_reports/FlashAttention/FlashDecode.md` | Current admitted bf16 flash-attention direct-runtime suite proves typed exact-CB publish / consume event windows, `acc_s -> acc_s_cast` materialization, physical-CB FIFO front-order safety, device tiled partial combine, seq32/64/128/256/512 MHA, GQA, paged/sparse/MLA decode, and split-block decode against host references | Multi-core split reduction beyond the admitted local subset, semaphore-backed remote writes, NoC traffic control, distributed decode scaling |
...
materialization, or T8 TIR-derived irregular work/access evidence.
Those primitives must be pulled forward explicitly instead of hidden under a
generic "later runtime" bucket.

## Queue Relationship

The active order, current state, and detailed runtime case matrix live in
`tasks/progress.md`.
This roadmap keeps only the stable resource-planning rules.

Completed T1-T5 surfaces are inputs to this roadmap, not duplicated status:
