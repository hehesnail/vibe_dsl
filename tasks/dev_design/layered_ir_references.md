...
- 文中若提到历史 phase / program 命名，
  默认按**历史研究输入**理解，
  不是当前活动设计的协议边界
- 当前活动协议只使用：
  `Normalized Tile TIR / SpatialPlan / TTProgram / ExecutableSpec`
- 现存 fake/legacy protocol 的显式名
  统一集中在
...
- 为什么 target builtin mapping 不能放到 tile-op / layout / 真实 load/store 语义被打碎之后
- 为什么 validator 不能只放在最后一层
- 为什么 `SpatialPlan` 和 `TTProgram` 不能混成一层 attr bag
...
- 用它帮助判断某个问题更像
  ...
  还是 target realization 表示
- 用它帮助审视 validator、映射边界、显式表示对象边界是否站得住

错误用法：

- 因为某篇论文用了某个 noun，就把它直接塞进当前长期表示字段
...
- 这一层更像一个 **bounded semantic abstraction**，而不是万能语义恢复器
...
- `Normalized Tile TIR` 之上的抽象层必须是稳定抽象
- access / effect / use-role / liveness 这类信息
  需要在显式表示对象 / validator 责任
  上显式化，
  而不是再长一层 facts bag
- `preserve / typed_rebind / invalidate` 这类表示层生命周期约束需要显式化

它最直接支持的是：

- 为什么算法/语义层要有显式 abstraction boundary
...
- 类型化的对象和跨层显式接口，远好于把所有层语义混进一层 attr bag

这支持了我们今天的三层分工：

- `Normalized Tile TIR` 固定算法与访存语义
- `SpatialPlan` 固定 virtual spatial/dataflow 表示
- `TTProgram` 固定 TT 资源与 ABI 表示
...
它和当前算法/语义层到 `SpatialPlan` 的关系最像的地方在于：
...
- semantic legality / refinement validation 不是装饰，它是主链 contract
- 后续 `ValidateSpatialPlan` / `ValidateTTProgram` 也是同类问题
...
- 如何把冻结后的算法语义组织成 `Task / Channel / Layout / WorkPartition / ProgramPhase / SyncEdge`
...
- `Task / Channel / Layout` 成为 `SpatialPlan` 一等对象
- `Task` 不默认等于 `TTKernel`
- `SpatialPlan` 先冻结 virtual spatial/dataflow 表示，再交给 `TTProgram`
...
- `SpatialPlan` 只能消费冻结后的语义表示
- `TTProgram` 不允许继续发明 task graph / phase 表示 / update law
...
- `Normalized Tile TIR` 管算法语义
- `SpatialPlan` 管空间组织表示

这也是为什么我们没有把 `Task / Channel / Layout` 塞回 `Normalized Tile TIR`。
...
## 5. `TTProgram` / Target Realization 参考

`TTProgram` 的问题和 `SpatialPlan` 已经不一样了。
它不再问“虚拟空间程序长什么样”，而是问：

- 这个 virtual program 如何落成 TT contract
- 哪些 kernel、CB、transport、sync、ABI object 必须显式存在
- 哪些信息属于 target realization 表示，哪些只是 materialization 结果
...
- 后续 `TTProgram / MaterializeBlackholeExecutable`

### 5.2 SPADA

`SPADA` 在 target realization 层仍然重要，因为它提醒我们：

- target-side routing / sync contract 不能退化成“代码能跑就算对”
- validation 应该继续存在于 target layer，而不是只在 spatial layer 结束

因此它既支撑 `SpatialPlan`，也支撑未来的 `ValidateTTProgram` 思路。

### 5.3 Spatial

`Spatial` 对 target realization 层的帮助更多是历史层面的：
...
- `Normalized Tile TIR` 固定算法语义
- `SpatialPlan` 固定 spatial/dataflow 表示
- `TTProgram` 固定 TT target realization 表示
...
- 为什么 `SpatialPlan` 不是 target attr bag
...
- 为什么 `TTProgram` 应是唯一 target realization 表示
- 为什么 `ExecutableSpec` 不应和 `TTProgram` 形成双入口
...
- 绕开当前仓库文档已经写清楚的边界
- 因为某篇论文方便，就把 paper-specific noun 硬塞进长期表示字段
- 把 target-specific 语义偷带回 `SpatialPlan`
- 把算法语义偷带回 `TTProgram`
