...
- `Normalized Tile TIR` 冻结算法与访存真相
- `SpatialPlan` 冻结 virtual spatial/dataflow truth
...
- `Task` 不默认等于 `TTKernel`
- `SpatialPlan` 先冻结 virtual truth，再交给 `TTProgram`
...
- `SpatialPlan` 只能消费冻结后的 semantic truth
- `TTProgram` 不允许继续发明 task graph / phase truth / update law
...
- `Normalized Tile TIR` 管 algorithmic truth
- `SpatialPlan` 管 spatial organization truth
...
- `Normalized Tile TIR` 冻结 algorithmic truth
- `SpatialPlan` 冻结 spatial/dataflow truth
- `TTProgram` 冻结 TT target truth
...
- 把 algorithmic truth 偷带回 `TTProgram`
