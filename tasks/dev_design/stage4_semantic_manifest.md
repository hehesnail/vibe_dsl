...
这个判断是对的。要解决它，不能只在 `blackhole_codegen` 里加一个“更聪明的恢复器”，而是要把 **显式语义在哪一层消失** 这件事在整条 lower 链上重新摆正。
...
### 4.1 结论：collector 不是一个点，而是两段 capture + 一次 projection
...
4. `AnalyzeSemanticStructure` 同时读取：
   ...
   - `blackhole.pipeline_stages`
   - `tl.semantic_seeds`
5. manifest 只替换 explicit-op update/witness 生成
6. `fragment_regions` 继续承接 residual recovery 与 `LowerBlackholeOps` 当前输入
