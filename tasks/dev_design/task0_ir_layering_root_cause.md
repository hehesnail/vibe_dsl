...
2. **“硬编码类型越加越多”**
   - `src/transform/common/companion_base.h` 已经累积了 6 个
     stringly-typed 命名空间：
     `schema_key::`（~90 条常量）
     `buffer_flow::`（state / stream / republish / logical_tile /
     write / compute_consume / transport_consume / reference）
     `buffer_materialization::`（`intermediate_accumulator_merge` /
     `republished_logical_tile` / `intermediate_buffer` /
     `republished_buffer` / `tile_nfaces_materialization` /
     `accumulator_delta` / `consumer_input` / `accumulator_add` /
     `direct_write` / `dst_cb_binary_pack` / `tiled_cb_republish`）
     `buffer_live_form::`（`tiled_cb`）
     `buffer_distribution_kind::`（`grouped_rows` / `row_state` /
     `thread_distributed`）
     `buffer_topology_kind::`（`linear`）
     `spatial_contract::`
   - `BufferMaterializationInfo` 当前形态：
     ```cpp
...
- 于是新增了一个补救机制：把这批 truth 先投影成
  ...
  再由 `AnalyzeBlackholeComputeRegions` 重新 materialize 成 typed
  `buffer_distribution_contracts`（见 `progress.md` 相关记录）
...
当前 IR（尤其是 companion schema 与 `BufferMaterializationInfo` /
...
- 没法问“这是什么 op”就只能贴“这是 `intermediate_accumulator_merge`”
  这样的 string 标签
- 而 `intermediate_accumulator_merge` 是一个**被命名的类别**，
