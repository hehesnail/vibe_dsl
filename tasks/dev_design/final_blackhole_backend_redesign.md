...
- **Phase B**: 仍在进行中
  - 已完成的子阶段：
    ...
    - `Spatial*` definitions / vocab / shared key 已从 semantic infra 拆出
    - `SpatialCapabilityModel` / `TTHardwareModel` 已落地为 module-scope global info
    - `Channel.kind + payload_kind + delivery_kind` 与 `placement.affinity_kind`
      已收成当前 translator probe intake 所需的最小 contract
  - 仍未完成的核心职责：
    - `SpatialProgram` 的 task formation / flow shaping / domain realization /
      phase-and-ordering synthesis / capability-informed legality-policy
      仍未整体收实
    - stronger execution-bearing contract 与 validator 职责仍未全部完成
- **Phase C**: 已定义；当前只有准备轨落地
  - read-only translator demand probe 与硬件模型 intake 已落地
  - 正式 `TT Target IR` / `MaterializeTTExecutableSpec` 真源 cutover
    仍受剩余 `Phase B` 工作阻塞

### 7.2 当前主 blocker

当前 blocker 仍然先落在 `Phase B`，然后才轮到正式 `Phase C` cutover：

- `Phase B` contract-hardening 子阶段已完成，但正文定义的 spatial synthesis algorithm
  与 stronger execution-bearing contract 仍未收实
- 因此 `SpatialProgram` 还不能视为最终形态的 virtual spatial program
- `TTProgram / MaterializeTTExecutableSpec` 仍不存在
- 当前 target/runtime 仍主要停留在 `LowerBlackholeOps -> PlanBlackholeCB -> AssignBlackholeCores`
  的旧 planning 主链
- `Spatial Program IR -> TT Target IR -> ExecutableSpec`
  的单一真源切换因此尚未进入正式实施
...
- 当前 blocker 仍先落在 `Phase B` 正文定义的 spatial synthesis 与 stronger contract
  收实，然后才是 `SpatialProgram -> TTProgram` 的正式切换
