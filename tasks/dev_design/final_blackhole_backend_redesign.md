...
- `kind` 不应成为 workload-specific 名词袋子。更稳的设计是：
  - 小闭枚举：
    ...
    - `control`
  - 细粒度语义落在 `traits`
    - 例如 `load / store / reduce / select / route / exchange / combine`
...
- `payload_kind` 只保留小闭枚举：
  - `data`
  - `index`
  - `state`
  - `control`
- `transport_semantics` 也只保留小闭枚举：
  - `point_to_point`
  - `broadcast`
  - `gather`
  - `scatter`
  - `reduce`
- workload-specific 细节应落在 `traits` 与 `payload_states`，例如：
  - `fifo`
  - `ordered`
  - `multicast`
  - `paged`
...
- `kind` 应是小闭枚举，`traits` 才承接 `tiled / sharded / grouped / routed / paged / ragged`
- `domain_bindings` 是 core 字段，不再只在后文 contract 里补充

**`WorkPartition`**

- `WorkPartition` 只表达“逻辑工作如何被切分”
- `kind` 应是小闭枚举，`traits` 再承接 `row / tile / pair / split_k / expert / page / chunk / selected_subset`
- `domain_bindings` 是 core 字段，用来保证 partition 不是脱离 semantic domain 的裸表达式

**`Placement`**

- 只表达 virtual placement
- `kind` 应保持小闭枚举，`traits` 再表达 `colocated / adjacent / row_group / column_group` 等偏好
...
- `kind` 只保留小闭枚举：
  ...
  - `barrier`
  - `completion`
- `traits` 再表达 `producer_consumer / multicast_ready / ordered_update / carry_completion`
- `update_or_state_bindings` 必须进入 core schema，而不是只在后文 contract 中出现

**`ResourceIntent`**

- `kind` 也应保持小闭枚举，`traits` 再表达 `transport_buffer / scratch / persistent_carry / reduction_carrier / index_buffer / output`
...
| 对象 | 作用 | 关键字段 |
|------|------|----------|
| `SpatialLegalityFacts` | analysis 产出的 must-have structure 与 legality constraints | `must_cut_boundaries`, `must_carry_edges`, `forced_phase_boundaries`, `forced_layout_families`, `forced_partition_families`, `required_sync_edges` |
