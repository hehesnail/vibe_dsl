...
- **日期**: 2026-03-19（创建），2026-04-03（重写并收敛为当前版本），2026-04-06（在 `Phase A` 完成后按当前分阶段结构再次精简，并完成一轮状态对齐审计），2026-04-07（按 `Phase B` compile-path 收口、contract hardening 持续进行与 `Phase C` 边界再次同步）
- **状态**: 当前唯一权威总体设计文档
...
- **Phase B**: compile-path cutover 已收口，但整体未结束
  ...
  - 更强的 contract / validator 仍是当前主实施重点
- **Phase C**: 已定义；以 `Phase B` contract hardening 达标为实现前提
  - `TT Target IR` / `MaterializeTTExecutableSpec` 真源切换尚未落地
...
- 当前 blocker 已转成 `SpatialProgram -> TTProgram` 的真源切换与 target materialization 收口
