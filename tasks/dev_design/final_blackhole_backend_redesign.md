...
- **日期**: 2026-03-19（创建），2026-04-03（重写并收敛为当前版本），2026-04-05（基于 review 与代码交叉审计修订）
- **状态**: 当前唯一权威总体设计文档
...
2. **companion IR 的 lift 点、失效规则、rebind contract 必须和真实 lowering 主链对齐**
   ...
   - 因此必须补齐：
     - 唯一允许的 semantic lift canonicalization 点
     - Phase A1 的 post-lift hard-freeze 规则
     - A2+ 才启用的 audited-safe pass 白名单
     - A2+ 才要求完整落地的 typed `TIRValueBinding` rebind API
     - 失效后必须整体删除并重建 `tl.semantic_program / tl.spatial_program / tl.tt_program` 的硬规则
...
- 只有当 Phase A2 实际证明这类分解无法稳定覆盖一个 workload family，
...
4. 一旦 `LiftToStatefulSemanticIR` 成功：
   - seed 可整体删除
...
2. 一旦某个 `PrimFunc` 挂上 `tl.semantic_program`，post-semantic-lift pass 默认都视为 `unsafe`；
   Phase A1 只允许 metadata/analysis 类 pass 继续通过，A2+ 才允许 audited-safe / rebind-aware pass。
...
Phase A1 的实际 pass window 直接固定为：

```text
CollectDevicePrograms
  -> SplitHostDevice
  -> BindSplitDeviceMembers
...
1. `LiftToStatefulSemanticIR` 与 `LowerToSpatialProgram` 之间默认不允许插入 TIR mutator
2. 如果 pipeline 中间必须插入新的 unsafe pass，则 builder 必须显式执行：
   - `InvalidateCompanionIR`
   - 重新跑 `AnalyzeSemanticStructure -> LiftToStatefulSemanticIR -> ValidateStatefulSemanticIR`
3. A1 不试图维护 companion IR 跨任意 TIR rewrite 存活；A1 先把 pass window 收紧

**Safe TIR mutations（不触发 companion IR 失效的改动类型）**：

实现顺序必须分阶段处理：

- **Phase A1** 采用更硬的规则：semantic lift 之后直到 `LowerToSpatialProgram`
  消费完 semantic truth 之前，不允许任何会改动被 companion IR 直接引用 TIR 原子的 mutation；
  也就是说，A1 不要求支持任意 rebind-aware safe pass
- **A2+** 如果确实需要在 lifted companion IR 之上继续运行 TIR 改写，
  才引入完整的 audited-safe / rebind-aware contract

Phase A1 safe 集必须**刻意收得很小**。原因不是这些改动一定会改变算法语义，而是
...
1. Phase A1 中每个 post-semantic-lift pass 只允许是：
   - metadata/analysis `safe`
   - 其他一律 `unsafe`
2. A2+ 才允许“identity-preserving”或“rebind-aware”的 audited pass 声明 `safe`
3. pipeline builder 依据声明插入 invalidation + re-lift

#### 5.6.3.1 固定 semantic lift canonicalization 点

“固定 canonicalization 点”不能只停留在原则描述上。第一版 contract 需要对齐当前主链，明确：

- semantic lift 的输入是 **generic TileLang/TVM lowering 结束后、TT-specific builtin lowering 开始前**
  的 device `PrimFunc`
- 对当前 Blackhole 主链，这个点至少要求已经完成：
  - `AnnotateBlackholeCopySemantics`
  - `BlackholeDeviceResourceCanonicalization`
  - `LowerOpaqueBlock`
  - `Simplify`
  ...
  - `SplitHostDevice`
- 如果程序包含多个 `T.Kernel` / 多 device region，则还要求：
  - pre-`SplitHostDevice` 的 device-program registry 已先建立
  - split 之后只是把 member function 绑定回 registry，而不是临时重新发明 program/phase truth
- 但还没有进入：
  - `SplitBlackholeKernel` 之后的 TT-specific compatibility planning 真源化
  - `LowerBlackholeOps`
  - `PlanBlackholeCB`
  - `AssignBlackholeCores`
  - `MaterializeTTExecutableSpec`

如果未来通用主链调整了 pass 顺序，这条规则也必须保持等价不变量：

1. copy/resource canonicalization 已稳定
2. device/private buffer scope 已 canonicalize
3. generic host/device 边界已稳定
4. 仍未进入 TT target contract 生成与物化

也就是说，semantic lift 的稳定输入是“generic canonicalized device program”，
不是 raw TIR，也不是已经开始发明 TT contract 的半物化程序。

#### 5.6.3.2 A2+ 的 `TIRValueBinding` / `TIRAnchor` rebind contract

当前 `BlackholeDeviceResourceCanonicalization` 仍存在 name-based fallback，这意味着在 Phase A1
阶段，指望 arbitrary TIR rewrite + 完整 typed rebind 一起落地并不现实。因此：

- Phase A1 可以直接禁止这类 pass
- 只有到 A2+ 确认确实需要 lifted companion IR 跨 TIR pass 存活时，
  才引入下面这套显式 rebind contract

届时，任何 post-semantic-lift audited-safe pass 都必须提供：
...
1. 加入最小 typed companion object 集：
   - `SemanticProgram / Domain / State / Update`
   - `AccessMap`
   - `UpdateLaw`
     - public schema 保持四类 variant
     - A1 full payload 只强制覆盖 `MapLaw / ReduceLaw`
     - non-attention skeleton case 只要求恢复正确的 `UpdateLaw.kind`
   - bridge helper：`TIRAnchor / TIRValueBinding`
   - `AtomicEffect / SemanticRegion / StateVersion / StateJoin`
     可先保持 compiler-internal helper struct，不要求 A1 就 TVM Object 化
2. 在 `LowerTileOp` 或更早的 canonicalization 位置补 compiler-internal early semantic capture channel，
   为 `AnalyzeSemanticStructure` 提供 lightweight semantic seed
3. 明确 `PrimFunc.attrs["tl.semantic_program"]` 挂载 contract
4. 落 `AnalyzeSemanticStructure`（最小版本）
   - 构造 `AtomicEffect`
...
5. 落 `LiftToStatefulSemanticIR`（最小版本）
6. 落 `ValidateStatefulSemanticIR`（核心验证规则）
7. Phase A1 采用 post-lift hard freeze：
   - semantic lift 之后直到 semantic truth 被消费完之前，不允许 TIR mutation
   - rebind-aware safe pass contract 推迟到 A2+
8. 要求现有 target lowering 从 semantic 真源消费信息，而不是继续自己猜主要语义
9. **验收 gate**：
   ...
   - 对这个 non-attention case，A1 只要求：
     - `Domain`
     - `State`
     - `UpdateLaw.kind`
     的恢复正确；完整 trait / rich payload 验证推到 A2
...
3. 落 `LowerToSpatialProgram`
   ...
   - 对 simple workload 提供 canonical fast-path：
     - 如果 `SemanticProgram` 退化为单个 dense `MapLaw` / `ReduceLaw`
     - 且不存在 carry / routed / paged / indirect / multi-phase 边界
     - 则直接构造 canonical `SpatialProgram`，不进入更重的 candidate synthesis
4. 落 `ValidateSpatialProgram`
5. 把 task/channel/layout/sync/work-partition 逻辑从 `LowerBlackholeOps` 中拆出去
6. **验收 gate**：
   - 至少一个 non-trivial multi-task / multi-phase workload 的 spatialization test 通过
     - `flash_decoding` split+combine 或 `fusedmoe` 优先
   - 至少一个 case 证明下游 TT mapping 不能再退化成“1 Task : 1 TTKernel”的实现心智模型

**Phase C: TT Target IR**

1. 加入 typed companion object：
   - `TTKernel / TTCoreGroup / TTCBPlan / TTTransportPlan / TTSemaphorePlan / TTComputeSyncPlan / TTDstLayoutPlan / TTABIPlan / TTExecutionPlan`
2. 在进入完整 target mapping 前，先建立 minimal `TTHardwareModel` stub，
   收拢当前散落在实现里的硬件常量与 legality 约束
3. 明确 `PrimFunc.attrs["tl.tt_program"]` 与 `IRModule.global_infos` 中 `hardware_model/topology` 的承载 contract
4. 落 `LowerSpatialProgramToTTTarget`
5. 落 `ValidateTTTargetProgram`
6. 从 `TT Target IR` 物化：
   - `ExecutableSpec`
   - `blackhole.segment_plan`
   - `blackhole.runtime_args`
...
7. 把 `PlanBlackholeCB`、`AssignBlackholeCores`、`rt_mod_blackhole` 收回到它们应有的 target/runtime 职责
