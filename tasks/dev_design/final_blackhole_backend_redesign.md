...
- `Domain.axes / constraints / predicate`
- direct / indirect `AccessMap`
- 普通 reduce / select / recurrence 骨架
- 基于 def-use 的 state lifetime

当以下语义无法仅靠结构稳定恢复时，必须允许前端或早期 IR 提供最小显式语义补充：

- 某个 `State` 的语义身份
- 某个 `AccessMap` 的高层语义 trait
- 某个 `UpdateLaw` 的 tie-break / algebraic trait / boundary policy
- 某个 ordered update 是否构成正式语义边界
...
- 某个 selection/update 骨架的 tie-break / algebraic trait / boundary policy
- 某个 ordered update 是否构成稳定 semantic boundary，而不是局部实现细节

这条边界的含义是：

1. 用户不负责告诉编译器“这里有 reduce / select / remap / carry”这类结构骨架。
2. 用户只在 IR 无法唯一裁决对象身份或关键 trait 时，补充最小 semantic fact。
3. DSL 补充的是 `Domain / State / Update` 的身份或 trait 信息，不是 workload family 名字，也不是 target 细节。
...
- 先验证 `flash-attn / topk / fusedmoe / paged decode / chunk recurrence`
  的主要结构骨架是否都能自动恢复
- 只有在某一类“身份裁决”或关键 `UpdateLaw` trait 被证明无法从 IR 稳定恢复时，
...
> IR 无法唯一裁决但编译又必需的语义身份或 trait，一律由前端 DSL 显式提供。

#### 统一语义系统的最小闭合集

Phase A 不能从第一天起把全部 policy、annotation protocol、helper view 一次性做满。
...
是 `Update` 的一等组成部分，而不是另一套平行 descriptor 家族。

其余内容都应作为：

- 这些对象的字段
- 这些对象上的 typed trait
- 或内部分析图 / 后续阶段的 policy / validation 输入
...
1. semantic 层不再维护 `CombineSpec / SelectionSpec / SegmentSpec / PageSpec / RecurrenceSpec`
   这类平行 descriptor 家族
2. Phase A1 先落 `Domain / State / Update`
3. Phase A2 再把 `AccessMap / UpdateLaw` 的更宽 trait 与 DSL annotation protocol 补齐
...
3. **形成候选 task**
   - 将同 algorithmic cut、同 domain family、同主要执行职责的一组 `Update`
     或导出 `SemanticRegion` 聚合为 `Task`
4. **生成 channel**
...
`anchor` 和 `SemanticRegion` 不能靠 workload 特判，也不能靠样例名猜。它们必须来自一套对复杂计算普遍成立的结构恢复规则。
...
`SemanticRegion` 定义为：**在同一 algorithmic cut 内，满足同一 domain family 与主要语义职责的一组最大连通 effect 节点。**

必须切 region 的情况：

1. `domain_signature` 改变
2. 主要 `effect_kind` 改变
3. 出现 loop-carried、cross-update 或 ordered-update 边
...
3. 落 `AnalyzeSemanticStructure`（最小版本）
   ...
   - 生成 anchor 与 `SemanticRegion` 导出视图
...
**Phase A2: 泛化 recovery + wider `AccessMap / UpdateLaw` traits + DSL annotation protocol**
...
5. `anchor / SemanticRegion` 恢复规则以通用结构分析为基础，而不是按样例名或 op 名做 case matcher
