...
## Parallel Theory Track

`Phase A` 的理论化/证明化内容已经单独抽到：

- [stage4_phase_a_formalization_note.md](/root/dev/vibe_dsl/tasks/dev_design/stage4_phase_a_formalization_note.md)

这个文档包含：

- canonical evidence / abstract semantic domain 的 formalization
- `E / A / alpha / R` 的半正式定义
- theorem/obligation checklist
- `Phase A -> Phase B` refinement interface
- `ValidateSpatialRefinement(SemanticProgram, SpatialProgram)` 的 skeleton

边界说明：

- `stage4_phase_a_semantic_ir.md` 保持为 **工程实现文档**
- `stage4_phase_a_formalization_note.md` 作为 **并行 research/formalization 文档**
- 后续 `Phase B` 实现不需要先等 formalization track 完成，除非显式要做
  refinement validator / research artifact
