...
- `Task / Channel`
  不再是 primary truth owner，只是 graph 上的 coarse grouping
- `SemanticProgram`
...
- `SpatialGraph`
  - owner：tile 级计算、逻辑对象、producer-consumer dependence、
    state/carry/partial merge、virtual task closure
- `TTProgram`
  - owner：block/resource/CB/semaphore/core/ABI/execution
- `Task / Channel`
  - 继续存在，但退成 derived execution/materialization view

### 4.3 `SpatialGraph` 的最小对象集

第一层 stable IR 的最小对象集合应当围绕“计算本质”而不是
GPU noun 建模：
...
- `spatial_dataflow_program_model.md`
  - 后续必须按 `SpatialGraph / VirtualTask / TTBlockPlan`
    边界重写
...
2. 在 `Simplify` 后建立新入口：
   - `ExtractSpatialGraph`
3. 在 graph 上建立 coarse execution grouping：
