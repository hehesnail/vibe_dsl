...
def rebuild_tt_program(
    ...
    *,
    ...
    return make_tt_program(
        ...
        list(program.reducer_plans) if reducer_plans is None else reducer_plans,
...
def block_indexed_sparse_3tile_ragged_staged_copy_kernel(
    grid_x: int,
    source_tiles: int,
    tile_m: int = 32,
    tile_n: int = 32,
    ...
    """Define a sparse three-tile copy with independent per-entry valid rows."""
    source_m = source_tiles * tile_m
    output_m = grid_x * 3 * tile_m

    if dtype == "bfloat16":
        @T.prim_func
        def main(
            A: T.Tensor((source_m, tile_n), "bfloat16"),
            BlockIndices: T.Tensor((grid_x, 3), "int32"),
            ValidRows: T.Tensor((grid_x, 3), "int32"),
            B: T.Tensor((output_m, tile_n), "bfloat16"),
        ):
            with T.Kernel(grid_x, 1) as (bx, by):
                A0_shared = T.alloc_shared((tile_m, tile_n), "bfloat16")
                A1_shared = T.alloc_shared((tile_m, tile_n), "bfloat16")
                A2_shared = T.alloc_shared((tile_m, tile_n), "bfloat16")
                tile0 = BlockIndices[bx, 0]
                tile1 = BlockIndices[bx, 1]
                tile2 = BlockIndices[bx, 2]
                valid0 = ValidRows[bx, 0]
                valid1 = ValidRows[bx, 1]
                valid2 = ValidRows[bx, 2]
                for i, j in T.Parallel(tile_m, tile_n):
                    A0_shared[i, j] = T.if_then_else(
                        i < valid0,
                        A[tile0 * tile_m + i, j],
                ...
                for i, j in T.Parallel(tile_m, tile_n):
                    A2_shared[i, j] = T.if_then_else(
                        i < valid2,
                        A[tile2 * tile_m + i, j],
                        0,
                    )
                T.copy(A0_shared, B[bx * 3 * tile_m, 0])
                T.copy(A1_shared, B[(bx * 3 + 1) * tile_m, 0])
                T.copy(A2_shared, B[(bx * 3 + 2) * tile_m, 0])

        return main

    raise ValueError(
        f"Unsupported block_indexed_sparse_3tile_ragged_staged_copy_kernel dtype: {dtype}"
    )
...
def segmented_three_range_masked_staged_copy_kernel(
    grid_x: int,
    source_rows: int,
    tile_m: int = 32,
    tile_n: int = 32,
    ...
    """Define a three-segment per-work row copy from start/count tables."""
    output_rows = grid_x * 3 * tile_m

    if dtype == "bfloat16":
        @T.prim_func
        def main(
            A: T.Tensor((source_rows, tile_n), "bfloat16"),
            SegmentOffsets: T.Tensor((grid_x, 3), "int32"),
            SegmentCounts: T.Tensor((grid_x, 3), "int32"),
            B: T.Tensor((output_rows, tile_n), "bfloat16"),
        ):
            with T.Kernel(grid_x, 1) as (bx, by):
                A0_shared = T.alloc_shared((tile_m, tile_n), "bfloat16")
                ...
                start0 = SegmentOffsets[bx, 0]
                start1 = SegmentOffsets[bx, 1]
                start2 = SegmentOffsets[bx, 2]
                count0 = SegmentCounts[bx, 0]
                count1 = SegmentCounts[bx, 1]
                count2 = SegmentCounts[bx, 2]
                for i, j in T.Parallel(tile_m, tile_n):
                    A0_shared[i, j] = T.if_then_else(
                        i < count0,
                        A[start0 + i, j],
    ...
    raise ValueError(f"Unsupported segmented_three_range_masked_staged_copy_kernel dtype: {dtype}")
...
def paged_valid_rows_masked_staged_copy_kernel(
    grid_x: int,
    pages_per_sequence: int,
    total_pages: int,
    tile_m: int = 32,
    ...
    """Define a paged copy guarded by a per-page valid-row table."""
    source_m = total_pages * tile_m
    output_m = grid_x * pages_per_sequence * tile_m
    ...
    if dtype == "bfloat16":
        ...
        def main(
            A: T.Tensor((source_m, tile_n), "bfloat16"),
            PageTable: T.Tensor((grid_x, pages_per_sequence), "int32"),
            PageValidRows: T.Tensor((grid_x, pages_per_sequence), "int32"),
            B: T.Tensor((output_m, tile_n), "bfloat16"),
        ):
            with T.Kernel(grid_x, pages_per_sequence) as (bx, by):
                A_shared = T.alloc_shared((tile_m, tile_n), "bfloat16")
                page_id = PageTable[bx, by]
                valid_rows = PageValidRows[bx, by]
                for i, j in T.Parallel(tile_m, tile_n):
                    A_shared[i, j] = T.if_then_else(
                        i < valid_rows,
                        A[page_id * tile_m + i, j],
    ...
    raise ValueError(f"Unsupported paged_valid_rows_masked_staged_copy_kernel dtype: {dtype}")
