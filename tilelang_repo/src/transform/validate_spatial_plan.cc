...
bool ContainsTTNoun(const std::string& value) {
  ...
  static const char* kForbidden[] = {
      "tt_",        "semaphore", "runtime_arg", "noc",
      "cb_",        "cb.",       "core_group",  "brisc",
      "trisc",      "ncrisc",    "ethernet",
  };
...
void ValidateNoTTNoun(const std::string& value, const std::string& context) {
  ICHECK(!ContainsTTNoun(value))
      << context << " must not contain TT-specific noun leakage: " << value;
}
...
std::vector<int> BuildPhaseIndexByUnit(const SpatialPlan& plan) {
  ...
  for (const PhasePlan& phase : plan->phase_plans) {
    ...
    for (int i = 0; i < phase->unit_indices.size(); ++i) {
      ...
      ICHECK_GE(unit_index, 0) << "PhasePlan " << phase->name << " requires non-negative unit_index";
  ...
  for (int unit_index = 0; unit_index < static_cast<int>(phase_index_by_unit.size()); ++unit_index) {
...
void ValidateExecutionUnits(const SpatialPlan& plan) {
  ...
  for (const ExecutionUnit& unit : plan->execution_units) {
    ...
    ICHECK(!unit->formation_basis.empty()) << "ExecutionUnit " << unit->name << " requires formation_basis";
    ICHECK(!unit->unit_role.empty()) << "ExecutionUnit " << unit->name << " requires unit_role";
    ICHECK(!unit->stmt_indices.empty()) << "ExecutionUnit " << unit->name << " requires stmt_indices";
...
void ValidateCompatibilityProjection(const SpatialPlan& plan) {
  ICHECK_EQ(plan->execution_units.size(), plan->closures.size())
      << "SpatialPlan compatibility projection requires execution_units/closures alignment";
  ICHECK_EQ(plan->dataflow_edges.size(), plan->boundaries.size())
      << "SpatialPlan compatibility projection requires dataflow_edges/boundaries alignment";
  ...
  for (int i = 0; i < plan->execution_units.size(); ++i) {
    ...
    ICHECK_EQ(str(unit->name), str(closure->name))
        << "SpatialPlan compatibility projection mismatch for ExecutionUnit name";
  ...
  for (int i = 0; i < plan->dataflow_edges.size(); ++i) {
    ...
    ICHECK_EQ(str(edge->name), str(boundary->name))
        << "SpatialPlan compatibility projection mismatch for DataflowEdge name";
    ICHECK_EQ(str(edge->kind), str(boundary->kind))
        << "SpatialPlan compatibility projection mismatch for DataflowEdge kind";
    ...
    ICHECK_EQ(edge->producer_unit_index, boundary->source_closure_index)
        << "SpatialPlan compatibility projection mismatch for producer_unit_index";
    ICHECK_EQ(edge->consumer_unit_index, boundary->target_closure_index)
        << "SpatialPlan compatibility projection mismatch for consumer_unit_index";
    ICHECK_EQ(str(edge->subject), str(boundary->subject))
        << "SpatialPlan compatibility projection mismatch for DataflowEdge subject";
    ICHECK(SameStringArray(edge->traits, boundary->traits))
        << "SpatialPlan compatibility projection mismatch for DataflowEdge traits";
  }
}
...
void ValidateDataflowEdges(const SpatialPlan& plan, const std::vector<int>& phase_index_by_unit) {
  ...
  for (const DataflowEdge& edge : plan->dataflow_edges) {
    ...
    const bool crosses_phase =
        phase_index_by_unit[edge->producer_unit_index] != phase_index_by_unit[edge->consumer_unit_index];
...
void ValidateLayoutSpecs(const SpatialPlan& plan) {
  ...
  for (int unit_index = 0; unit_index < static_cast<int>(plan->execution_units.size()); ++unit_index) {
    const ExecutionUnit& unit = plan->execution_units[unit_index];
  ...
  for (const LayoutSpec& layout : plan->layout_specs) {
    ...
    for (int i = 0; i < layout->unit_indices.size(); ++i) {
      ...
      ICHECK_GE(unit_index, 0) << "LayoutSpec " << layout->name << " requires non-negative unit_index";
      ICHECK_LT(unit_index, static_cast<int64_t>(plan->execution_units.size()))
...
void ValidatePhasePlans(const SpatialPlan& plan, const std::vector<int>& phase_index_by_unit) {
  ...
  for (const PhasePlan& phase : plan->phase_plans) {
    ...
    for (int i = 0; i < phase->ingress_edge_indices.size(); ++i) {
      ...
      ICHECK_GE(edge_index, 0) << "PhasePlan " << phase->name << " ingress edge index must be non-negative";
      ...
      ICHECK(edge->crosses_phase)
          << "PhasePlan " << phase->name << " ingress edges must be cross-phase";
    ...
    for (int i = 0; i < phase->egress_edge_indices.size(); ++i) {
      ...
      ICHECK_GE(edge_index, 0) << "PhasePlan " << phase->name << " egress edge index must be non-negative";
      ICHECK_LT(edge_index, plan->dataflow_edges.size())
      ...
      ICHECK(edge->crosses_phase)
          << "PhasePlan " << phase->name << " egress edges must be cross-phase";
