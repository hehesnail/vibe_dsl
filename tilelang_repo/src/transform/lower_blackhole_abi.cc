...
class SegmentLeafCollector final : public tir::StmtVisitor {
 public:
  void VisitStmt_(const tir::EvaluateNode *op) final {
    leaves.push_back(GetRef<Stmt>(op));
  }
  ...
  std::vector<Stmt> leaves;
};

static std::vector<Stmt> CollectSegmentLeafStmts(const Stmt &body) {
  SegmentLeafCollector collector;
  if (body.defined()) {
    collector(body);
  }
...
static Map<String, Any> MakeRuntimeArg(const std::string &name,
  ...
  if (requires_per_work_binding) {
    arg.Set(blackhole_runtime_arg_schema::kRequiresPerWorkBinding,
            Bool(true));
  }
  return arg;
}
...
static Map<String, Any> MarkRuntimeArgRequiresPerWorkBinding(
  ...
  arg.Set(blackhole_runtime_arg_schema::kRequiresPerWorkBinding, Bool(true));
  return arg;
}
      ...
      if (existing_index >= 0) {
        runtime_args.push_back(
            requires_per_work_binding
                ? MarkRuntimeArgRequiresPerWorkBinding(
...
static TTRuntimeArgSpec DecodeRuntimeArgSpec(const Any &item) {
  ...
  return TTRuntimeArgSpec(
      ...
      GetMapBool(
          arg, blackhole_runtime_arg_schema::kRequiresPerWorkBinding));
}
...
void PlanTTKernelABI::LoadSeededSegmentBodies(const PrimFunc &func) {
  seeded_segment_bodies_by_kind_.clear();
  auto staged_program = func->GetAttr<TTProgram>(attr::kTLTTProgram);
  if (!staged_program) {
    return;
  }
  std::unordered_map<std::string, std::vector<Stmt>> seed_leaves_by_kind;
  for (const TTKernel &kernel : staged_program.value()->kernels) {
    const std::string kind = kernel->kind;
    if (kind.empty() || !kernel->body.defined()) {
      continue;
    }
    if (seeded_segment_bodies_by_kind_.find(kind) ==
        seeded_segment_bodies_by_kind_.end() &&
        recorded_segment_nodes_by_kind_.find(kind) ==
  ...
  if (seed_leaves_by_kind.empty()) {
    return;
  }
  std::unordered_map<std::string, std::vector<bool>> matched_by_kind;
  for (const auto &entry : seed_leaves_by_kind) {
    matched_by_kind.emplace(entry.first, std::vector<bool>(entry.second.size(), false));
  }
  const std::vector<Stmt> current_leaves = CollectSegmentLeafStmts(func->body);
  StructuralEqual equal;
  auto recorded_by_other_segment = [&](const Stmt &stmt, const std::string &kind) {
    for (const auto &entry : recorded_segment_nodes_by_kind_) {
      if (entry.first != kind && entry.second.count(stmt.get()) != 0U) {
        return true;
      }
    }
    return false;
  };
  for (const Stmt &leaf : current_leaves) {
    for (const std::string &kind : recorded_segment_kind_order_) {
      auto seed_it = seed_leaves_by_kind.find(kind);
      if (seed_it == seed_leaves_by_kind.end()) {
        continue;
      }
      if (recorded_by_other_segment(leaf, kind)) {
        continue;
      }
      std::vector<bool> &matched = matched_by_kind[kind];
      const std::vector<Stmt> &seed_leaves = seed_it->second;
      for (size_t i = 0; i < seed_leaves.size(); ++i) {
        if (matched[i]) {
          continue;
        }
        if (equal(leaf, seed_leaves[i])) {
          recorded_segment_nodes_by_kind_[kind].insert(leaf.get());
          matched[i] = true;
          break;
        }
      }
...
void PlanTTKernelABI::StoreRecordedSegmentKernelSeeds() {
  tt_kernels_.clear();
...
void PlanTTKernelABI::StoreSegmentPlan(PrimFunc &func) {
  const std::vector<std::string> segment_kinds = recorded_segment_kind_order_;
  const std::unordered_map<std::string, Stmt> segment_bodies =
      BuildRecordedSegmentBodies();
  ...
  ICHECK(!requires_compute_segment_ || !segment_kinds.empty())
      << "PlanTTKernelABI requires explicit segment body truth for "
...
void PlanTTKernelABI::StoreAccessorDescriptors(PrimFunc &func) {
  ...
  auto make_segment_per_work_arg_specs = [&](const std::string &kind,
                                             ...
                                             const Array<Any> &segment_accessors,
                                             const Optional<Any>
                                                 &existing_specs_opt) {
    Array<TTPerWorkArgSpec> per_work_arg_specs =
        existing_specs_opt
    ...
    auto runtime_arg_buffer_for_kind =
        [&](const char *arg_kind) -> std::string {
      for (const Any &arg_item : runtime_args) {
        auto arg = arg_item.as<Map<String, Any>>().value_or(Map<String, Any>());
        if (!arg.Get("kind") || static_cast<std::string>(Downcast<String>(
        ...
        if (auto buffer = arg.Get("buffer")) {
          return Downcast<String>(buffer.value());
        }
        return "";
      }
      return "";
    ...
    auto tile_start_value_source_for_buffer =
      ...
      for (const Any& accessor_item : segment_accessors) {
        auto accessor =
            accessor_item.as<Map<String, Any>>().value_or(Map<String, Any>());
        if (accessor.empty() || !accessor.Get("buffer") ||
            static_cast<std::string>(Downcast<String>(
                accessor.Get("buffer").value())) != buffer_name) {
          continue;
        }
        const Array<Integer> host_axis_order =
            GetMapIntegerArray(accessor, "host_axis_order");
        if (host_axis_order.size() >= 2U &&
            host_axis_order[0].IntValue() == 0 &&
            host_axis_order[1].IntValue() == 2) {
          return blackhole_runtime_arg_schema::kValueSourceLogicalBlockYXLinear;
        }
        break;
      }
      return default_value_source;
    ...
    if (kind == "reader") {
      ...
      const std::string runtime_a_buffer =
          runtime_arg_buffer_for_kind("a_tile_start_id");
      const std::string runtime_b_buffer =
          runtime_arg_buffer_for_kind("b_tile_start_id");
      const std::string a_tile_buffer =
          !runtime_a_buffer.empty() ? runtime_a_buffer : gemm_a_buffer_name_;
    ...
    if (kind == "writer") {
      const std::string runtime_output_buffer =
          runtime_arg_buffer_for_kind("output_tile_start_id");
      ...
      if (auto accessor_items = segment.Get("accessors")) {
        for (const auto &accessor_item :
          ...
          for (const AccessorDescriptor &desc : accessor_descriptors_) {
            ...
            if (matched_descriptor != nullptr) {
              ICHECK(matched_descriptor->layout == desc.layout &&
                     ...
                     matched_descriptor->host_axis_order ==
                         desc.host_axis_order &&
                     matched_descriptor->transpose_2d == desc.transpose_2d)
                  << "PlanTTKernelABI found conflicting accessor descriptors "
                     "for buffer "
                  << buffer_name
                  << "; a single accessor slot cannot mix transport metadata";
              continue;
            }
          ...
          if (matched_descriptor != nullptr) {
            ...
            if (!matched_descriptor->host_axis_order.empty()) {
              if (!resolved_host_axis_order.empty()) {
                ICHECK(resolved_host_axis_order ==
                       matched_descriptor->host_axis_order)
                    << "PlanTTKernelABI found conflicting host_axis_order for "
                    << buffer_name;
              } else {
                resolved_host_axis_order = matched_descriptor->host_axis_order;
              }
            }
            resolved_transpose_2d =
                resolved_transpose_2d || matched_descriptor->transpose_2d;
          }
          ...
          if (!resolved_host_axis_order.empty()) {
            Array<Any> axis_order;
            for (int64_t axis : resolved_host_axis_order) {
              axis_order.push_back(Integer(axis));
            }
            accessor.Set("host_axis_order", axis_order);
          }
          if (resolved_transpose_2d) {
