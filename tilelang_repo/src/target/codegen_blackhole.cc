...
void CodeGenBlackhole::GenerateKernelMain(const tvm::GlobalVar &gvar,
  ...
  // Note: We don't call parent AddFunction because we handle the entry point differently
  // Instead, we process the function body directly

  // Set up thread indices for Blackhole (single core execution)
  // Map block/thread indices to constants for single-core mode
  // Note: Thread extent processing is handled by VisitStmt_ for AttrStmtNode
