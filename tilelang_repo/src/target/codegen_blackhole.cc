...
std::string CodeGenBlackhole::GetKernelCode() const {
  // Return only the kernel code without TVM headers from decl_stream
  // We only return stream.str() which contains our TT-Metal kernel code
  // The decl_stream contains TVM headers added by CodeGenCHost::Init
  return stream.str();
}

void CodeGenBlackhole::AddFunction(const tvm::GlobalVar &gvar,
                                   const tvm::tir::PrimFunc &f) {
  // Always emit TT-Metal headers for kernel code
  // Note: We use static flags to avoid duplicate headers when multiple functions are added
  ...
  if (!headers_emitted) {
    ...
    if (global_symbol) {
      ...
      if (symbol.find("_brisc") != std::string::npos) {
        ...
        core_type_ = CoreType::kNCRISC;
                 ...
                 symbol.find("_compute") != std::string::npos) {
        core_type_ = CoreType::kTRISC;
      }
    }
    ...
    switch (core_type_) {
      ...
      case CoreType::kTRISC:
        ...
        break;
      default:
        // Default to dataflow API for simple kernels
  ...
  // Generate TT-Metal kernel_main function instead of standard C function
  GenerateKernelMain(gvar, f);
}

void CodeGenBlackhole::GenerateKernelMain(const tvm::GlobalVar &gvar,
                                          const tvm::tir::PrimFunc &f) {
  // Get function name for comments
  auto global_symbol = f->GetAttr<tvm::ffi::String>(tvm::attr::kGlobalSymbol);
  std::string func_name = global_symbol ? static_cast<std::string>(global_symbol.value())
                                        : static_cast<std::string>(gvar->name_hint);

  // Detect kernel type from function body
  // For simple copy kernels (B = A), generate optimized CB/NOC code
  bool is_simple_copy = DetectSimpleCopyKernel(f);

  if (is_simple_copy) {
    GenerateCopyKernelMain(f, func_name);
  } else {
    GenerateGenericKernelMain(f, func_name);
  }
}

bool CodeGenBlackhole::DetectSimpleCopyKernel(const tvm::tir::PrimFunc &f) {
  // Check if this is a simple copy kernel (B[i] = A[i] pattern)
  // This is a heuristic - checks for single buffer store with buffer load
  // For now, assume it's a copy if it has 2 buffer params (input + output)
  int buffer_params = 0;
  for (const auto &param : f->params) {
    if (param->dtype.is_handle()) buffer_params++;
  }
  return buffer_params == 2;
}

void CodeGenBlackhole::GenerateCopyKernelMain(const tvm::tir::PrimFunc &f,
                                               const std::string &func_name) {
  // Add TT-Metal headers to stream (not decl_stream to avoid TVM headers)
  stream << "// TT-Metal kernel generated by TileLang\n";
  ...
  int num_tiles = 1;  // Default to 1 tile for simple copy

  for (size_t i = 0; i < f->params.size(); ++i) {
    const auto &param = f->params[i];
    if (param->dtype.is_handle()) {
      if (src_buf.empty()) {
        src_buf = param->name_hint;
      } else {
        dst_buf = param->name_hint;
      }
    }
  }

  // Add function name as comment
  stream << "// Copy Kernel: " << func_name << "\n";
  stream << "// Uses CB0 for data transfer from " << src_buf << " to " << dst_buf << "\n";

  // Generate kernel_main entry point
  stream << "void kernel_main() {\n";

  // Argument loading
  stream << "  // Load kernel arguments from runtime\n";
  stream << "  uint64_t src_addr = ((uint64_t)get_arg_val<uint32_t>(1) << 32) | get_arg_val<uint32_t>(0);\n";
  stream << "  uint64_t dst_addr = ((uint64_t)get_arg_val<uint32_t>(3) << 32) | get_arg_val<uint32_t>(2);\n";
  stream << "  uint32_t num_tiles = get_arg_val<uint32_t>(4);\n";
  stream << "\n";

  // CB configuration
  stream << "  // CB configuration\n";
  stream << "  constexpr uint32_t cb_id = 0;\n";
  stream << "  constexpr uint32_t tile_size = 2048;  // 32x32 FP16\n";
  stream << "\n";

  // Address generators
  stream << "  // Address generators for DRAM access\n";
  stream << "  InterleavedAddrGen<true> src_gen = {\n";
  stream << "    .bank_base_address = (uint32_t)src_addr,\n";
  stream << "    .page_size = tile_size\n";
  stream << "  };\n";
  stream << "  InterleavedAddrGen<true> dst_gen = {\n";
  stream << "    .bank_base_address = (uint32_t)dst_addr,\n";
  stream << "    .page_size = tile_size\n";
  stream << "  };\n";
  stream << "\n";

  // Reader: DRAM -> CB
  stream << "  // Reader: DRAM -> CB\n";
  stream << "  for (uint32_t i = 0; i < num_tiles; i++) {\n";
  stream << "    cb_reserve_back(cb_id, 1);\n";
  stream << "    uint32_t write_ptr = get_write_ptr(cb_id);\n";
  stream << "    uint64_t src_noc_addr = get_noc_addr(i, src_gen);\n";
  stream << "    noc_async_read(src_noc_addr, write_ptr, tile_size);\n";
  stream << "    noc_async_read_barrier();\n";
  stream << "    cb_push_back(cb_id, 1);\n";
  stream << "  }\n";
  stream << "\n";

  // Writer: CB -> DRAM
  stream << "  // Writer: CB -> DRAM\n";
  stream << "  for (uint32_t i = 0; i < num_tiles; i++) {\n";
  stream << "    cb_wait_front(cb_id, 1);\n";
  stream << "    uint32_t read_ptr = get_read_ptr(cb_id);\n";
  stream << "    uint64_t dst_noc_addr = get_noc_addr(i, dst_gen);\n";
  stream << "    noc_async_write(read_ptr, dst_noc_addr, tile_size);\n";
  stream << "    noc_async_write_barrier();\n";
  stream << "    cb_pop_front(cb_id, 1);\n";
  stream << "  }\n";
