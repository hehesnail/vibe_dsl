...
本次v2.0版本进行了重大更新，整合了 [METALIUM_GUIDE.md](https://github.com/tenstorrent/tt-metal/blob/main/METALIUM_GUIDE.md) 官方指南的独特内容，并基于 [TT-Metal Tech Reports](https://github.com/tenstorrent/tt-metal/tree/main/tech_reports) 进行了深度增强：

### v2.0 新增内容

| 章节 | 新增内容 | 来源 |
|------|---------|------|
| **前言** | Executive Summary 执行摘要、目标读者指南 | METALIUM_GUIDE |
| **第1章** | 新增1.8节"硬件架构专家视角"：GPU专家视角、CPU专家视角、缓存层次结构详解 | METALIUM_GUIDE |
| **第1章** | SRAM和缓冲区详解（Interleaved vs Sharded模式） | METALIUM_GUIDE |
| **第2章** | Tile原生计算深度说明、与GPU张量核心对比 | METALIUM_GUIDE |
| **第3章** | 完整的向量加法示例（Host端+Kernel端） | METALIUM_GUIDE |
| **第7章** | Compute API跨代兼容性说明 | METALIUM_GUIDE |
| **第8章** | Fast Dispatch vs Slow Dispatch详解 | METALIUM_GUIDE |
...
**标注说明**:
- **【METALIUM指南】** 标记: 来自官方 METALIUM_GUIDE.md 的内容
- **【技术报告补充】** 标记: 来自 TT-Metal Tech Reports 的内容
...
- [前言](#前言)
  ...
  - [学习路径](#学习路径)
- [第1章: 架构概述](#第1章-架构概述)
  - [1.1 什么是 TT Metalium](#11-什么是-tt-metalium)
  - [1.2 支持的硬件代际](#12-支持的硬件代际)
  - [1.3 核心设计原则](#13-核心设计原则)
  - [1.4 多芯片架构概述](#14-多芯片架构概述)
  - [1.5 子设备架构](#15-子设备架构)
  - [1.6 内存层次结构](#16-内存层次结构)
  - [1.7 NoC 通信机制](#17-noc-通信机制)
  - [1.8 硬件架构专家视角](#18-硬件架构专家视角)
- [第2章: 核心概念](#第2章-核心概念)
- [第3章: 编程示例](#第3章-编程示例)
...
本指南向开发者介绍 Tenstorrent AI 加速器（称为 Tensix 处理器）架构和 TT-Metalium™ 编程模型。与传统 GPU 依赖大规模线程并行不同，Tensix 处理器使用由专用计算节点组成的网格，每个节点称为 Tensix 核心。

每个 Tensix 核心包含：
- **5个小型 RISC-V CPU**：用于控制和指令分发
- **专用硬件单元**：矩阵运算单元 (FPU) 和向量运算单元 (SFPU)
- **数据打包/解包单元**：优化数据格式转换
- **1.5MB 本地 SRAM**：高速片上存储

**典型数据流**：
```
DRAM → NoC接口 → 解包器 → 计算单元(FPU/SFPU) → 打包器 → NoC → DRAM/其他Tensix
```

这种设计优先考虑高效的数据移动和本地 SRAM 使用，减少对 DRAM 的频繁访问。

**三 Kernel 协作模型**：

Metalium 编程通常需要每个 Tensix 核心运行三种 Kernel：
1. **Reader Kernel（读取内核）**：负责数据输入
2. **Compute Kernel（计算内核）**：执行实际计算
3. **Writer Kernel（写入内核）**：负责数据输出

这些 Kernel 通过 SRAM 中的循环缓冲区（Circular Buffer）进行协调。架构原生操作 32×32 的 Tile，针对深度学习操作进行了优化。

Metalium 提供 API 和抽象层（包括计算和数据移动）来简化开发、管理硬件资源，并确保 Kernel 跨硬件代的兼容性。
...
| 读者类型 | 背景要求 | 学习目标 |
|----------|----------|----------|
| **GPU 开发者** | 熟悉 CUDA/OpenCL | 理解 Tenstorrent 与 GPU 的架构差异 |
| **CPU 开发者** | 熟悉多核/众核编程 | 掌握显式内存管理和 DMA 操作 |
| **AI 系统程序员** | 有底层优化经验 | 开发高性能算子 Kernel |
| **框架开发者** | 了解编译器/运行时 | 构建 Tenstorrent 软件栈上层工具 |
...
2. [第1章 - 架构概述](#第1章-架构概述)
3. [第2章 - 核心概念](#第2章-核心概念)
4. [第3章 - Hello World 示例](#31-hello-world-kernel)

**系统学习** (1-2周):
1. 完成快速上手内容
2. [第4-7章 - 完整 API 参考](#第4章-host-api-参考)
3. [第3章 - 所有编程示例](#第3章-编程示例)
4. [第1.8节 - 硬件架构专家视角](#18-硬件架构专家视角)

**深度开发** (持续):
1. [第8章 - 性能优化指南](#第8章-性能优化指南)
2. [第9章 - 调试工具详解](#第9章-调试工具详解)
3. 实践中查阅 API 参考
...
本节从不同硬件架构背景的开发者视角，解释 Tenstorrent 硬件的核心设计特点。

### 1.8.1 GPU 专家视角

与现代 GPU 架构通过大规模线程并行和延迟隐藏技术实现高吞吐量不同，Tenstorrent 处理器采用了一种根本不同的架构方法，以异步 I/O 操作和高效的资源利用为核心。

**关键架构差异**：

| 特性 | GPU | Tenstorrent Tensix |
|------|-----|-------------------|
| **线程模型** | 大规模线程并行 (SIMT) | 单线程执行，无硬件多线程 |
| **延迟隐藏** | 通过线程切换隐藏延迟 | 显式 DMA 操作，软件流水线 |
| **执行模式** | 协作式线程调度 | RISC-V 核心直接控制硬件单元 |
| **缓存层次** | 多级透明缓存 | **零硬件缓存**，显式 SRAM 管理 |
| **内存访问** | 透明内存管理 | 显式 DMA 操作 |
| **互连** | 专用互联 (NVLink) | 片上网络 (NoC) 支持近内存计算 |

**核心区别详解**：

1. **无 SIMT 设计**：Tensix 中的 Baby RISC-V 核心不使用单指令多线程 (SIMT) 抽象。相反，它们直接发出命令到专门的硬件单元（矩阵/向量单元）执行实际计算。

2. **显式 DMA**：不同于 GPU 的透明内存访问，Tensix 要求开发者显式使用 DMA 操作（通过 NoC）在 DRAM 和 SRAM 之间移动数据。

3. **确定性内存访问**：没有缓存层次意味着内存访问延迟是可预测和一致的，消除了缓存未命中的性能抖动。

4. **近内存计算**：NoC 互连允许数据在本地 SRAM 中处理，无需频繁访问 DRAM。

### 1.8.2 CPU 专家视角

Tenstorrent 架构与 IBM CELL 处理器架构类似，具有以下特点：

**架构相似性**：

| 特性 | CELL 处理器 | Tenstorrent Tensix |
|------|------------|-------------------|
| **控制核心** | PPE (Power Processing Element) | Baby RISC-V CPU |
| **计算核心** | SPE (Synergistic Processing Elements) | FPU/SFPU 专用单元 |
| **本地存储** | 256KB Local Store 每 SPE | 1.5MB SRAM 每 Tensix |
| **内存访问** | 显式 DMA | 显式 NoC DMA |
| **缓存** | 无传统缓存 | **零硬件缓存** |
| **协作模式** | SPE 协作完成任务 | 三 Kernel 协作模型 |

**关键特点**：

1. **专用 SRAM**：每个 Tensix 包含 1.5MB 专用 SRAM，Baby RISC-V 核心和外围设备直接访问。不在本地 SRAM 中的数据必须通过 DMA 显式传输。

2. **显式内存管理**：由于 DRAM 位于 Tensix 地址空间之外，需要通过显式 DMA 操作访问。这消除了内存一致性和延迟管理的复杂性。

3. **协作式核心**：Tensix 内的核心协作完成任务，而不是独立处理不同的工作部分。三个计算核心（Unpack/Math/Pack）通过硬件互斥机制和循环缓冲区结构进行通信。

4. **简约 RISC-V**：Baby RISC-V 核心是简化的 RISC-V 32IM 处理器，无任务切换能力，专注于指令分发而非复杂计算。

### 1.8.3 缓存层次结构在哪里？

**【METALIUM指南】**

Tenstorrent 芯片**故意省略**了传统 CPU 和 GPU 架构中的缓存层次结构。

**设计哲学**：

```
传统架构 (CPU/GPU)              Tenstorrent 架构
─────────────────────          ─────────────────────
┌─────────────────┐            ┌─────────────────┐
│   L1 Cache      │            │                 │
│  (透明缓存)      │            │   1.5MB SRAM    │
...
1. **直接 SRAM 访问**：整个芯片提供对 SRAM 的直接访问。任何组件都可以读取或写入位于其他 Tensix 核心中的 SRAM。

2. **显式数据移动**：数据移动需要显式操作——要么其他核心将数据写入目标 Tensix 的 SRAM，要么 Tensix 本身发出请求获取所需数据。

3. **无缓存一致性协议**：消除了复杂的缓存一致性协议开销。

4. **可预测的内存访问延迟**：内存访问延迟是可预测和一致的，没有缓存命中/未命中的不确定性。

5. **开发者责任**：开发者需要有效地管理数据放置和移动模式，但获得了完全的控制权和可预测性。

**对比总结**：

```
CPU/GPU:  透明缓存 ← 自动管理 ← 硬件控制 ← 不可预测
           ↓
         缓存未命中 → 性能抖动

Tensix:   显式 SRAM ← 程序员控制 ← 直接寻址 ← 完全可预测
           ↓
         显式 DMA → 确定性延迟
```

### 1.8.4 SRAM、交织缓冲区与分片缓冲区

**【METALIUM指南】**

每个 Tensix 包含 1.5MB SRAM，每个芯片有数百个 Tensix 核心，提供了相当大的片上内存容量。

**混合内存架构**：

与纯 SRAM 架构（要求所有数据必须适配 SRAM）不同，Tenstorrent 采用混合方法：
- 数据可以根据需要存储在 SRAM 或 DRAM 中
- 中间结果可以保留在 SRAM 中，无需驱逐压力
- 当操作可以访问同一 Tensix 内本地存储的数据时，完全消除 NoC 带宽需求

**缓冲区配置模式**：

| 模式 | 描述 | 适用场景 |
|------|------|----------|
| **Interleaved (交织)** | 数据分布到所有可用 DRAM 控制器 | 通用工作负载，均衡性能 |
| **Sharded (分片)** | 张量根据芯片物理拓扑分布在多个 Tensix 核心 | 注意力机制、卷积操作 |

**Interleaved vs Sharded**：

```
Interleaved Mode:                    Sharded Mode:
┌─────────────────┐                 ┌─────┐┌─────┐┌─────┐
│  DRAM Bank 0    │                 │Tensix││Tensix││Tensix│
│  DRAM Bank 1    │                 │  0   ││  1   ││  2   │
│  DRAM Bank 2    │                 └──┬──┘└──┬──┘└──┬──┘
│  DRAM Bank 3    │                    │      │      │
└─────────────────┘                    └──────┴──────┘
...
Sharded 内存放置对以下场景特别有益：
- **注意力机制**：查询-键-值计算模式与数据分布策略对齐
- **卷积操作**：数据局部性与卷积核访问模式匹配
- **减少片外通信**：最小化跨芯片通信开销
...
### 1.1 Compute API 跨代兼容性

**【METALIUM指南】**

Compute API 抽象层解决了一个根本问题：**在不同硬件代际之间保持内核代码的兼容性和性能**。

**为什么需要 Compute API？**

Tenstorrent 硬件在不同代际之间有显著的架构变化：

| 硬件代际 | 向量宽度 | 浮点精度 | 特殊指令 |
|----------|----------|----------|----------|
| **Grayskull** | 64 元素 | 19-bit 浮点 | 基础指令集 |
| **Wormhole** | 32 元素 | 32-bit 浮点 | float_to_int16 |
| **Blackhole** | 32 元素 | 32-bit 浮点 | 增强指令集 |

**问题**：如果没有 Compute API 抽象，开发者需要为特定硬件代际编写内核，导致：
- **代码脆弱性**：为 Grayskull 编写的内核（使用64宽向量操作）无法在 Wormhole 或 Blackhole 上运行
- **维护开销**：每个新硬件代际都需要重写和重新验证内核

**Compute API 解决方案**：

Compute API 提供稳定的编程接口。当从计算内核调用 API（如 `sin_tile`）时，Metalium 编译器自动选择针对目标硬件向量宽度、指令集和其他能力优化的正确实现。

```cpp
// 同样的代码适用于所有硬件代际
sin_tile(0);  // Grayskull、Wormhole、Blackhole 自动选择不同实现
```

**示例：sin_tile 的不同实现**

Grayskull 实现（64元素向量，4次迭代）：
```cpp
...
Blackhole/Wormhole 实现（32元素向量，8次迭代）：
```cpp
...
inline void calculate_sine() {
    for (int d = 0; d < ITERATIONS; d++) {
        ...
        vInt whole_v = float_to_int16(v, 0);  // 使用 Blackhole 特殊指令
...
**重要提示**：虽然 Metalium 支持自定义向量化计算实现，但请注意不同处理器代际之间的向量宽度和硬件特定细节各不相同。对于大多数应用，请使用提供的 Compute API，因为 Tenstorrent 会维护这些实现以确保与未来硬件代的兼容性。
...
Tenstorrent 硬件原生操作 32×32 元素的 Tile，这优化了常见的深度学习操作，如矩阵乘法和卷积。

**Tile 布局的优势**：

传统线性内存布局在矩阵操作中存在根本低效：执行矩阵操作时，硬件必须缓冲整行（通常数千个元素），因为访问下一行需要跳过大的内存偏移，或者必须手动实现锯齿形/分块矩阵乘法，这增加了复杂性。

```
线性布局 (传统 CPU/GPU):          Tile 布局 (Tenstorrent):
┌─────────────────────┐          ┌─────────────────────┐
│ Row 0: 0 1 2 3 ...  │          │ Tile (0,0): 0-31    │
│ Row 1: N N+1 N+2... │          │    每32元素换行     │
│ ...                 │          │ Tile (0,1): 32-63   │
│ Row M: ...          │          │ ...                 │
└─────────────────────┘          └─────────────────────┘
       ↓                                 ↓
需要大缓冲区存储整行             仅需32×32=1024元素缓冲区
跳转大偏移量                     下一行元素仅隔32元素
```
...
| 架构 | 矩阵操作方式 | 缓冲需求 | 效率 |
|------|-------------|----------|------|
| **CPU** | 线性访问 + 软件分块 | 大缓存 | 中等 |
| **GPU** | 张量核心 + 软件 Tile | 中等缓存 | 高 |
| **Tenstorrent** | 原生 Tile 操作 | 最小 SRAM | **极高** |

**与 GPU 张量核心的对比**：

GPU 在高效为张量核心提供数据方面经常遇到困难，因为它们的架构主要围绕向量操作设计。这可能导致张量工作负载的内存访问模式不够优化。

相比之下，Tenstorrent 硬件从一开始就专为基于 Tile 的计算而构建——向量单元和矩阵单元都在原生 32×32 Tile 上操作。这种方法带来更可预测的性能和更简单的编程模型。

**Tile 化 (Tilization)**：

- 张量以 Tile 格式原生存储在 Tenstorrent 处理器上
- 当返回主机系统时转换回线性格式
- 例如：48×1024 矩阵由于 32×32 Tile 对齐要求变成 64×1024 矩阵，第一维填充 16 行
- Tile 化和去 Tile 化操作可以在设备上执行，减少模型加载和推理期间的 CPU 开销

**性能数据**：

详细的性能数字请参考：
- [矩阵乘法性能报告](https://github.com/tenstorrent/tt-metal/blob/main/tech_reports/GEMM_FLOPS/GEMM_FLOPS.md)
- [Tenstorrent 芯片上的卷积网络](https://github.com/tenstorrent/tt-metal/blob/main/tech_reports/CNNs/ttcnn.md) 报告
...
## 9. Fast Dispatch 与 Slow Dispatch

**【METALIUM指南】**

Metalium 提供两种命令分派模式：Fast Dispatch（快速分派）和 Slow Dispatch（慢速分派）。理解这两种模式的区别对于性能优化至关重要。

### 9.1 Fast Dispatch（快速分派）

**工作原理**：

Fast Dispatch 通过为设备上的每个队列分配一个专用的 RISC-V 核心来实现异步命令队列。与 GPU 使用专门的片上调度器不同，Tenstorrent 为每个命令队列分配一个 RISC-V 核心来处理队列操作。这个核心通常放置在未使用的以太网 Tile 上，这样可以最小化对计算性能的影响。
...
│                      Device (Tensix)                         │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  Dispatch Core (RISC-V on Ethernet Tile)             │   │
│  │  - Fetches commands from host memory                 │   │
│  │  - Executes independently                            │   │
│  │  - Frees CPU to do other work                        │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```

**优势**：
- **异步执行**：命令存储在主机内存中，设备独立获取和执行
- **CPU 解放**：CPU 可以处理其他工作（或空闲以降低功耗），而设备处理命令
- **更好利用率**：更好的整体系统利用率

### 9.2 Slow Dispatch（慢速分派）

**工作原理**：

当通过设置 `TT_METAL_SLOW_DISPATCH_MODE=1` 环境变量禁用 Fast Dispatch 时，异步分派机制被绕过。这禁用了命令队列功能，并阻止主机和设备之间的异步 I/O 操作。

在 Slow Dispatch 模式下，CPU 必须等待每个操作完成才能继续执行下一个指令。必须使用同步 API 调用（如 `ReadFromBuffer`）而不是异步对应函数（如 `EnqueueReadMeshBuffer`）。

**缺点**：
- **CPU 开销**：主机 CPU 必须主动管理所有操作，包括与设备之间的数据传输
- **同步等待**：主机在等待设备操作完成时无法执行其他任务
- **性能降低**：显著的性能开销
...
| 场景 | 推荐模式 | 说明 |
|------|----------|------|
| **生产工作负载** | Fast Dispatch | 始终使用 Fast Dispatch |
| **调试分派问题** | Slow Dispatch | 仅当怀疑分派机制本身故障时使用 |
| **诊断问题来源** | Slow Dispatch | 确定问题来自命令队列系统还是内核执行 |

**代码对比**：

```cpp
// Fast Dispatch - 异步，CPU 可并行工作
EnqueueReadMeshBuffer(queue, buffer, host_ptr, /*blocking=*/false);
// CPU 可以在这里执行其他工作...

// Slow Dispatch - 同步，CPU 必须等待
...
与 OpenCL 的命令队列（可选支持乱序执行）不同，Tenstorrent 的命令队列架构由于 Baby RISC-V 核心的性能特性，强制执行严格顺序执行。

为了实现计算和数据传输操作的重叠，每个设备提供两个专用命令队列：
- **Queue 0**：约定用于处理计算工作负载
- **Queue 1**：约定用于管理数据传输操作
...
本示例演示如何使用 Metalium API 实现向量加法操作，包含完整的 Host 端代码和 Kernel 端代码。这是学习 TT-Metalium 编程的最佳入门示例。

**问题描述**: 计算两个输入缓冲区 A 和 B 的逐元素和，每个包含 `n_tiles` 个 Tile，将结果写入输出缓冲区 C。
...
│  (Data Move 0)  │ CB0│ (TRISC0-2)      │ CB1│ (Data Move 1)   │
└─────────────────┘    └─────────────────┘    └─────────────────┘
       │                                              │
       └────────────── DRAM ↔ L1 ─────────────────────┘
```
...
void kernel_main() {
    ...
    for (uint32_t i = 0; i < n_tiles; i++) {
        ...
        noc_async_read_barrier();  // 等待 Tile 读取完成
...
**说明**: 同一个源代码用于为所有三个 Unpack/Math/Pack Baby RISC-V 核心创建二进制。Metalium 计算 API 使用仅在特定类型核心上启用的代码段实现。编译器会自动将计算内核代码编译三次，每次针对不同的核心。

#### Writer Kernel (写入内核)

**文件**: `writer_kernel.cpp`
...
int main() {
    ...
    CBHandle cb_in0 = CreateCircularBuffer(
        ...
        CircularBufferConfig(
            ...
            {{cb_in0_index, tt::DataFormat::Float16_b}})
    ...
    CBHandle cb_in1 = CreateCircularBuffer(
        ...
        CircularBufferConfig(
            tiles_per_cb * tile_size_bytes,
            {{cb_in1_index, tt::DataFormat::Float16_b}})
    ...
    CBHandle cb_out = CreateCircularBuffer(
        ...
        CircularBufferConfig(
            tiles_per_cb * tile_size_bytes,
            {{cb_out_index, tt::DataFormat::Float16_b}})
